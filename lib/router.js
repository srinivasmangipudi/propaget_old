...
DashboardController = RouteController.extend({
	...
	agentSubscriptions: function() {
		Meteor.subscribe('singleAgentByUserId', this.myId()).wait();
		Meteor.subscribe('leadCount', this.agentId()).wait();
	},
	clientSubscriptions: function() {
		Meteor.subscribe('singleClientByUserId', this.myId()).wait();
	},	
	leadCount: function() {
		return Counts.get('leadsCount');
	},
	myId: function() {
	...
	client: function() {
		return Clients.findOne({userId: this.myId()});
	},
	agentId: function() {
	...
	data: function() {
		...
		if(user)
		{
			...
			return {
				...
				agent: this.agent(),
				leads: this.leads(),
				ready: this.agentSub.ready && this.leadsSub.ready,
